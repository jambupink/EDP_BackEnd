...
namespace LearningAPI.Controllers
{
    ...
    public class DonationController : ControllerBase
    {
        ...
        [HttpDelete("delete/{donationId}"), Authorize]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> DeleteDonation(int donationId)
        {
            try
            {
                var donation = await context.Donations.FindAsync(donationId);
                if (donation == null)
                {
                    return NotFound(new { message = "Donation not found." });
                }
            ...
            {
                logger.LogError(ex, "Error while deleting donation");
                return StatusCode(500, new { message = "An error occurred while deleting the donation." });
            }
        }
